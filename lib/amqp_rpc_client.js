    ...
    send: function send(routingKey, message) {
      ...
      return new Promise(function(resolve, reject) {
        amqplib.connect(url).then(function(conn) {
          console.log('[AMQP RCP CLIENT] connected to ', url);
          var corrId = uuid();
          return conn.createChannel()
            .then(function(channel) {
              ...
              return channel.assertExchange(exchangeName, exchangeType, {
                ...
                .then(function(queue) {
                  console.log('[AMQP RCP CLIENT] publishing', exchangeName, routingKey);
                  channel.publish(exchangeName, routingKey, new Buffer(JSON.stringify(message)), {

    ...
    send: function send(routingKey, message) {
      ...
      function consumeOnce(channel, queue, correlationId) {
        return new Promise(function(resolve, reject) {
          console.debug('[AMQP RCP CLIENT] consume inside promise', queue, correlationId);
          try {
            channel.consume(queue, function(msg) {
              console.debug('[AMQP RCP CLIENT] received reply');
      ...
      return new Promise(function(resolve, reject) {
        amqplib.connect(url).then(function(conn) {
          console.debug('[AMQP RCP CLIENT] connected');
          var corrId = uuid();
          return conn.createChannel()
            .then(function(channel) {
              console.debug('[AMQP RCP CLIENT] got channel');
              return channel.assertExchange(exchange, exchangeType, {
                durable: false
              })
                .then(function() {
                  console.debug('[AMQP RCP CLIENT] creating exclusive queue');
                  return exclusiveQueue(channel).then(function(queue) {
                    return queue;
                  });
                })
                .then(function(queue) {
                  console.debug('[AMQP RCP CLIENT] setting up consumer');
                  consumeOnce(channel, queue, corrId)
                    .then(function(reply) {
                      resolve(JSON.parse(reply.content.toString()));
                    }).
                  finally(function() {
                    console.debug('[AMQP RCP CLIENT] closing connection');
                    conn.close();
                  });
                  return queue;
                })
                .then(function(queue) {
                  console.debug('[AMQP RCP CLIENT] publishing', exchange, routingKey);

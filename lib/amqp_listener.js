...
function Listener(amqplib) {
  return Object.create({
    listen: function listen(url, queueName, exchangeName, exchangeType, callback, prefetchValue) {
      ...
      return amqplib.connect(url)
        ...
        .then(Listener.createConsumer(queueName, exchangeName, exchangeType, callback, prefetchValue));
    }
  });
}

Listener.createConsumer = createConsumer;
function createConsumer(queueName, exchangeName, exchangeType, callback, prefetchValue) {
  prefetchValue = prefetchValue || 0;

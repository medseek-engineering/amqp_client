    ...
    send: function send(routingKey, message, publishOpts) {
      ...
      return new Promise(function (resolve, reject) {
        amqplib.connect(url).then(function (conn) {
          console.debug('[AMQP 1W CLIENT] connected');
          return conn.createChannel().then(function (channel) {
            console.debug('[AMQP 1W CLIENT] got channel');
            var ok = channel.assertExchange(exchangeName, exchangeType, {durable: false});
            return ok.then(function () {
              console.debug('[AMQP 1W CLIENT] publishing', exchangeName, routingKey);
          ...
          .then(function () {
            console.debug('[AMQP 1W CLIENT] closing connection');
            conn.close();
          });
        })
        .catch(function (err) {
